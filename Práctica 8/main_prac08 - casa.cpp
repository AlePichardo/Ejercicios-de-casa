...
//Flecha Arriba: Rotar en +Y
//Flecha Abajo: Rotar en -Y
//Flecha Derecha: Rotar en +X
//Flecha Izquierda: Rotar en -X
...
void myData()
{
	...
	unsigned char *data = stbi_load("Texturas/marmol01.png", &width, &height, &nrChannels, 0);
	if (data)
	{
		// IMPORTANT note
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// load image, create texture and generate mipmaps
	data = stbi_load("Texturas/ajedrez.png", &width, &height, &nrChannels, 0);
	if (data)
	{
...
void display(void)
{
	...
	modelTemp = model;
	model = glm::scale(model, glm::vec3(3.5f, 0.1f, 3.5f));
	projectionShader.setMat4("model", model);

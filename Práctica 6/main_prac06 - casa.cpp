...
void animate(void)
{
	nucleo += 0.5f; //Para que gire solo, no depende de una tecla
	electron += 0.3f;
...
void display(void)
{
	...
	model = glm::scale(model, glm::vec3(0.4f, 0.4f, 0.4f));
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(1.0f, 0.3f, 0.0f));
	sphere_draw();
	model = glm::mat4(1.0f);

	//Electrón 6
	model = glm::rotate(model, glm::radians(electron), glm::vec3(1.0f, 1.0f, 0.0f)); //Movimiento de Traslación
	model = glm::translate(model, glm::vec3(0, 0, 6));
